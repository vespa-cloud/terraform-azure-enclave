...
resource "azurerm_storage_container" "node_archive" {
  storage_account_id    = azurerm_storage_account.archive.id
  name                  = var.zone.__enclave_infra.tenant_name
  container_access_type = "private"
}
...
resource "azurerm_role_assignment" "id_tenant_archive_writer" {
  ...
  role_definition_id = var.zone.__enclave_infra.archive_writer_role_resource_id
  principal_id       = var.zone.__enclave_infra.id_tenant_principal_id
}
