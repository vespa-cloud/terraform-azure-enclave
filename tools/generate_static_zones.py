    ...
    for i, line in enumerate(lines):
        ...
        if start is not None and "default" in line:
            ...
            if depth == 0:
                break
    if not capture:
        sys.exit("❌ Could not find __all_zones default block.")
    block = "[" + "\n".join(capture)
...
def parse_zones(block: str):
    ...
    try:
        ...
        sys.exit(f"❌ Failed to parse default block: {e}")
...
def write_outputs(path: Path, data):
    with open(path, "w", encoding="utf-8") as f:
        ...
        f.write("# --------------------------------------------------------------------\n")
        f.write("# 🚀 AUTOGENERATED FILE: DO NOT EDIT MANUALLY\n")
        ...
        for env in env_keys:
            ...
            for rkey in sorted(regions.keys()):
                ...
                f.write('        enclave_infra = module.provision.enclave_infra\n')
                f.write("      }\n")
            f.write("    }\n")
        f.write("  }\n")
        f.write("}\n")
